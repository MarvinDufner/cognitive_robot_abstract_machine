...
from semantic_digital_twin.adapters.urdf import (
    URDF,
    Collision,
    Mesh,
    Box,
...
from semantic_digital_twin.utils.geometry import (
    cube_volume,
    cube_surface,
    sphere_volume,
    cylinder_volume,
...
from semantic_digital_twin.utils.hash import get_file_hash

from semantic_digital_twin.world_description.geometry import Color


class LinkGeometry:
    link_T_geometry: HomogeneousTransformationMatrix
    color: Color

    def __init__(
        self,
        link_T_geometry: Optional[HomogeneousTransformationMatrix] = None,
        color: Optional[Color] = None,
    ):
        self.color = color or Color(20 / 255, 27.1 / 255, 80 / 255, 0.2)
        self.link_T_geometry = link_T_geometry or HomogeneousTransformationMatrix()
    ...
    def as_urdf(self):
        raise NotImplementedError(
            f"as_urdf not implemented for {self.__class__.__name__}"
        )

    @classmethod
    def from_urdf(cls, urdf_thing: Collision, color: Color) -> LinkGeometry:
        urdf_geometry = urdf_thing.geometry
        if urdf_thing.origin is None:
            link_T_geometry = HomogeneousTransformationMatrix()
        else:
            link_T_geometry = HomogeneousTransformationMatrix.from_xyz_rpy(
                xyz=urdf_thing.origin.xyz,
                rpy=urdf_thing.origin.rpy,
                reference_frame=None,
            )
        if isinstance(urdf_geometry, up.Mesh):
            geometry = MeshGeometry(
                link_T_geometry=link_T_geometry,
            ...
            raise NotImplementedError(
                f"{type(urdf_geometry)} geometry is not supported"
            )
        return geometry
...
class MeshGeometry(LinkGeometry):
    def __init__(
        ...
        if not os.path.isfile(resolved_file_name):
            raise Exception(f"Can't find file {resolved_file_name}")
...
class BoxGeometry(Shape):
    ...
    def is_big(
        ...
        return (
            cube_volume(self.depth, self.width, self.height) > volume_threshold
            or cube_surface(self.depth, self.width, self.height) > surface_threshold
        )
...
class CylinderGeometry(LinkGeometry):
    ...
    def is_big(
        ...
        return (
            cylinder_volume(self.radius, self.height) > volume_threshold
            or cylinder_surface(self.radius, self.height) > surface_threshold
        )
...
class Link:
    ...
    def from_urdf(cls, urdf_link: up.Link, prefix: str, color: ColorRGBA) -> Link:
        ...
        for urdf_collision in urdf_link.collisions:
            link.collisions.append(
                LinkGeometry.from_urdf(urdf_thing=urdf_collision, color=color)
            )
        for urdf_visual in urdf_link.visuals:
            link.visuals.append(
                LinkGeometry.from_urdf(urdf_thing=urdf_visual, color=color)
            )
        return link
    ...
    def as_urdf(self) -> str:
        robot = URDF(name=str(self.name))
        robot.version = "1.0"
        link = Link(name=str(self.name))
        link.collision = Collision(geometry=self.collisions[0].as_urdf())
        robot.add_link(link)
    ...
    def has_collisions(
        ...
        for collision in self.collisions:
            geo = collision
            if geo.is_big(
                volume_threshold=volume_threshold, surface_threshold=surface_threshold
            ):
                return True
        return False
